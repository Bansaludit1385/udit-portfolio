    ...
    <div className="w-full flex justify-center">
      <motion.nav
        ...
        className={`${scrolled ? 'bg-primary/40' : 'bg-primary/25'} 
          ...
          ${
            hovering
              ? 'shadow-[0_15px_40px_-10px_rgba(100,255,218,0.4)]'
      ...
      >
        <div className="flex justify-between items-center w-full">
          ...
          >
            <Link href="/" className="relative group">
              <span className="relative z-10">Udit Bansal</span>
            </Link>
          </motion.div>
          ...
          >
            {navItems.map((item, index) => (
              ...
              >
                <Link
                  href={`#${item}`}
                  className={`nav-item relative group overflow-hidden ${
                    activeSection === item ? 'text-accent' : 'text-textLight'
                  }`}
                ...
                >
                  ...
                  <span className="absolute top-0 left-0 w-full h-full bg-accent/5 scale-0 group-hover:scale-100 rounded-md transition-transform duration-300 -z-10" />
                </Link>
              </motion.div>
            ))}
          </motion.div>
            ...
            {[
              {
                ...
                icon: <FaGithub size={20} className="sm:text-xl" />,
              },
              {
                href: 'https://www.linkedin.com/in/udit-bansal-8288ba294/',
                icon: <FaLinkedin size={20} className="sm:text-xl" />,
              },
              {
                href: 'https://x.com/UditBan52859684',
                icon: <FaTwitter size={20} className="sm:text-xl" />,
              },
            ].map((social, index) => (
              <motion.a
                key={index}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            <div className="py-4 px-6 flex flex-col space-y-3">
              {navItems.map((item) => (
                <motion.div key={item} variants={mobileItemVariants} className="py-2">
                  <Link
                    href={`#${item}`}
